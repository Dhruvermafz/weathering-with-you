...
(async function () {
  ...
  console.log(coords);
  axios
    .get(`${process.env.REACT_APP_WEATHER_APP_API_URL}/api/location/name/home`)
    .then((response) => {
      response.data.length === 0
        ? axios.post(
            `${process.env.REACT_APP_WEATHER_APP_API_URL}/api/location`,
            {
              latitude: coords.coords.latitude,
              longitude: coords.coords.longitude,
              type: "Point",
              coordinates: [coords.coords.latitude, coords.coords.longitude],
              name: "home",
            }
          )
